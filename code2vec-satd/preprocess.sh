...
${PYTHON} JavaExtractor/extract.py --dir ${TRAIN_DIR} --max_path_length 8 --max_path_width 2 --num_threads ${NUM_THREADS} --jar ${EXTRACTOR_JAR} | gshuf > ${TRAIN_DATA_FILE}
echo "Finished extracting paths from training set"

#clean leave lines that starts with [satd , fixed]
clean_lines(){
  sort $1 > $1.full
  rm $1
  ${PYTHON} preprocess-satd.py $1.full $1 || exit 1
#  grep -E '^fixed|^satd' $1.full | sort > $1
  echo $1 leaving only satd or fixed, line count before/after: $(wc -l < $1.full)/ $(wc -l < $1)
}

clean_lines ${VAL_DATA_FILE}
clean_lines ${TEST_DATA_FILE}
clean_lines ${TRAIN_DATA_FILE}
#clean end
...
echo disabled following command: rm ${TRAIN_DATA_FILE} ${VAL_DATA_FILE} ${TEST_DATA_FILE} ${TARGET_HISTOGRAM_FILE} ${ORIGIN_HISTOGRAM_FILE} \
  ${PATH_HISTOGRAM_FILE}
