...
public class LeavesCollectorVisitor extends TreeVisitor {
	...
	private static boolean isGenericParent(Node node) {
	...
	private static boolean hasNoChildren(Node node) {
		return node.getChildrenNodes().size() == 0;
	}
	
	private static boolean isNotComment(Node node) {
	...
	private static int getChildId(Node node) {
